...
H3N2_forecasts <- purrr::pmap(list(res = forecast_res, name = 1:length(forecast_res),
                                   year = tasks$start), ~mutate(..1$df, forecast = ..2) %>% filter(Time > ..3 + 40/52)) %>%
  dplyr::bind_rows()
...
p_H3N2_Ne_forecast <- visualize_flu_season(H3N2_Ne_data, time_series_names = "H3N2_Ne", 
  ...
  geom_vline(aes(xintercept = 5), linetype = "dashed", alpha = 0.3) + # week 45
  geom_vline(aes(xintercept = 14), linetype = "dashed", alpha = 0.3) + # week 1
  geom_vline(aes(xintercept = 23), linetype = "dashed", alpha = 0.3) # week 10
...
H1N1_forecasts <- purrr::pmap(list(res = forecasts, name = 1:length(forecast_res),
                                   year = tasks$start), ~mutate(..1$df, forecast = ..2) %>% filter(Time > ..3 + 40/52)) %>%
  dplyr::bind_rows()
