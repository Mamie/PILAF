...
H3N2_all <- dplyr::bind_rows(skygrid, 
...
skygrid <- system.file("extdata/H1N1/skygrid_reconstruction.tsv", package = "PILAF")
skygrid <- readr::read_tsv(skygrid, skip = 1) %>%
  mutate(Time = H1N1_last_time - Time) %>% 
  mutate(Label = "SkyGrid")

BNPR_traj <- phylodyn::BNPR(H1N1_MCC_phylo)
BNPR_PS_traj <- phylodyn::BNPR_PS(H1N1_MCC_phylo)
H1N1_all <- dplyr::bind_rows(skygrid, 
...
skygrid <- system.file("extdata/Victoria/skygrid_reconstruction.tsv", package = "PILAF")
skygrid <- readr::read_tsv(skygrid, skip = 1) %>%
  mutate(Time = Victoria_last_time - Time) %>% 
  mutate(Label = "SkyGrid")

BNPR_traj <- phylodyn::BNPR(Victoria_MCC_phylo)
BNPR_PS_traj <- phylodyn::BNPR_PS(Victoria_MCC_phylo)
Victoria_all <- dplyr::bind_rows(skygrid, 
...
skygrid <- system.file("extdata/Yamagata/skygrid_reconstruction.tsv", package = "PILAF")
skygrid <- readr::read_tsv(skygrid, skip = 1) %>%
  mutate(Time = Yamagata_last_time - Time) %>% 
  mutate(Label = "SkyGrid")

BNPR_traj <- phylodyn::BNPR(Yamagata_MCC_phylo)
BNPR_PS_traj <- phylodyn::BNPR_PS(Yamagata_MCC_phylo)
Yamagata_all <- dplyr::bind_rows(skygrid, 
...
```{r}
pdf("~/Desktop/H3N2_H1N1_B_Victoria_Yamagata.pdf", width = 5, height = 8)
print(PILAF::plot_MCC_Ne(H3N2_MCC_tree, H3N2_root_time, H3N2_last_time, H3N2_all, main = "A/H3N2", breaks = seq(2013, 2019), c(1.3, 3)))
print(PILAF::plot_MCC_Ne(H1N1_MCC_tree, H1N1_root_time, H1N1_last_time, H1N1_all, main = "A/H1N1", breaks = seq(2013, 2019), c(1, 3)))
print(PILAF::plot_MCC_Ne(Victoria_MCC_tree, Victoria_root_time, Victoria_last_time, Victoria_all, main = "B/Victoria", breaks = seq(2013, 2019), c(1, 3)))
print(PILAF::plot_MCC_Ne(Yamagata_MCC_tree, Yamagata_root_time, Yamagata_last_time, Yamagata_all, main = "B/Yamagata", breaks = seq(2013, 2019), c(1, 3)))
dev.off()
```
