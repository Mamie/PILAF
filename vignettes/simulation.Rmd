...
p <- plot(sim.data, alpha = 0.1)
ggsave(p, file='../../projectingvirus/manuscript/Figures/simulation/simulatedTraj.pdf', width = 5, height = 4)
```

Forecast the trajectories. 

```{r forecasting, message=FALSE, warning=FALSE, fig.width=6, fig.height=5}
train.time = seq(0, 150) # given 101 weeks before and including present
...
count_fit = forecast(sim.train, test.time, PILAF:::Convert2Week(test.time), return_model = T, verbose = T)
plot(count_fit$forecast, sim.train, truth=sim.test, alpha = 0.1)
```
...
count_fit_week = forecast(sim.train, test.time, PILAF:::Convert2Week(test.time),
                          formula = formula, return_model = T, verbose = T)
...
```{r}
count_fit_week$models[[1]]$summary.hyperpar
```
...
joint_fit_week = forecast(sim.train, test.time, PILAF:::Convert2Week(test.time),
                          formula = formula, return_model = T, method = 'joint', verbose=T)
...
```{r}
joint_fit_week$models[[1]]$summary.hyperpar
```

```{r}
p <- PILAF::compare(count_fit_week$forecast, joint_fit_week$forecast, sim.test, method = c('count only', 'joint'))
p
ggsave(p, file = '../../projectingvirus/manuscript/figures/simulation/simulation-results.pdf', width = 5, height = 3)
```
